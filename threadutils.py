        ...
        while True:
            ...
            for url in self.processlistdict["waitingurllist"]:
                if not url in self.processlistdict["finishedurllist"]:
                    ...
                    html=content.text
                    rex = r'<a title="Older Comments" href="(http://jandan.net/ooxx/page\-\d+#comments)" class="previous-comment-page">»</a>'

                    doc = lxml.html.fromstring(content.text)
                    urllist = re.findall(rex,html)
                    ...
                    #图片的读取
                    srllist = doc.xpath(self.confdict["imgxpath"])
                    imgsrclist = srllist[4:]
